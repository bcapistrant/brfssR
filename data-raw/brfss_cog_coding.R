...
COG_2015_1 <- COG_2015_1 %>%
  ...
  mutate(YEAR=2015,
         cog_version="X_LCPWTV1",
         COG_wt_raw=X_LCPWTV1)

COG_2015_2<-read.xport("data-raw/LLCP15V1.XPT")
COG_2015_2 <- COG_2015_2 %>%
  filter(X_STATE %in% COG2015_2) %>%
  mutate(YEAR=2015,
         cog_version="X_LCPWTV2",
...
COG_2016_1<-read.xport("data-raw/LLCP15V1.XPT")
...
COG_2016_2<-read.xport("data-raw/LLCP15V1.XPT")
...
COG_2016_3<-read.xport("data-raw/LLCP15V1.XPT")
COG_2016_3 <- COG_2016_3 %>%
  filter(X_STATE %in% COG2016_3) %>%
  mutate(YEAR=2016,
         cog_version="X_LCPWTV3",
...
COG_2017_2<-read.xport("data-raw/LLCP15V1.XPT")
...
COG_2017_3<-read.xport("data-raw/LLCP15V1.XPT")
...
list2017<-list(COG_2017_0, COG_2017_1,COG_2017_2,COG_2017_3)
...
brfss_COG$X_STATE<- factor(brfss_COG$X_STATE, levels=COGstates, labels=COGstatelabel)
...
brfss_COG <- brfss_COG %>%
  mutate(cog_mem_d_fct=as.factor(if_else(CIMEMLOS==1, "Yes",
         ...
         cog_hous_d_fct=as.factor(if_else(CDHOUSE %in% 1:3, "Sometimes+",
                                 if_else(CDHOUSE %in% 4, "Rarely_Never", NA_character_))),
         ...
         cog_asst_d_fct=as.factor(if_else(CDASSIST %in% 1:3, "Sometimes+",
                                 if_else(CDASSIST %in% 4, "Rarely_Never", NA_character_))),
         ...
         cog_help_d_fct=as.factor(if_else(CDHELP %in% 1:3, "Sometimes+",
                                 if_else(CDHELP %in% 4, "Rarely_Never", NA_character_))),
         ...
         cog_social_d_fct=as.factor(if_else(CDSOCIAL %in% 1:3, "Sometimes+",
                                 if_else(CDSOCIAL %in% 4, "Rarely_Never", NA_character_))),
