...
#' @examples
#' # To adjust the sampling weight (cog_wt_raw) by dividing the sampling weight by the number of instances a state is in the data, run:
#' library(dplyr)
#' library(tidyr)
#' var_data_stateyears <- brrfss_covariates %>%
#'   filter(year %in% 2016:2017) %>%
#'   mutate(wave = count(state,year),
#'        cog_wt_adj = cog_wt_raw/wave
#'   )
"brfss_cog"
